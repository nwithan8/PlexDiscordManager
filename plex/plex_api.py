...
from modules.connectors.ombi import OmbiConnector
from modules.connectors.tautulli import TautulliConnector
...
class TautulliCredentials:
    def __init__(self, tautulli_settings_entry):
        self.enabled = getattr(tautulli_settings_entry, "Enabled", False)
        self.url = getattr(tautulli_settings_entry, "URL")
        self.api_key = getattr(tautulli_settings_entry, "ApiKey")


class OmbiCredentials:
    def __init__(self, ombi_settings_entry):
        self.enabled = getattr(ombi_settings_entry, "Enabled", False)
        self.url = getattr(ombi_settings_entry, "URL")
        self.api_key = getattr(ombi_settings_entry, "ApiKey")


class PlexCredentials:
    def __init__(self, plex_settings_entry, tautulli_settings_entry, ombi_settings_entry):
        self.plex_url = plex_settings_entry.URL
        self.plex_token = plex_settings_entry.Token
        self.plex_server_name = plex_settings_entry.Name
        self.plex_server_alt_name = plex_settings_entry.AltName
        self.terminate_message = plex_settings_entry.TerminateMessage
        self.tautulli = (TautulliCredentials(tautulli_settings_entry=tautulli_settings_entry) if tautulli_settings_entry else None)
        self.ombi = (OmbiCredentials(ombi_settings_entry=ombi_settings_entry) if ombi_settings_entry else None)
...
class PlexInstance:
    ...
    def set_tautulli_connection(self, tautulli_info):
        self.use_tautulli = tautulli_info.enabled
        if self.use_tautulli:
            self.tautulli = TautulliConnector(url=tautulli_info.url, api_key=tautulli_info.api_key)

    def set_ombi_connection(self, ombi_info):
        self.use_ombi = ombi_info.enabled
        if self.use_ombi:
            self.ombi = OmbiConnector(url=ombi_info.url, api_key=ombi_info.api_key)
...
class PlexConnections:
    def __init__(self, plex_credentials: PlexCredentials, database: DiscordMediaServerConnectorDatabase):
        """
    ...
    def plex_connectors(self) -> dict:
        connectors = {1: PlexInstance(url=self._credentials.plex_url,
                                      token=self._credentials.plex_token,
                                      api=self,
                                      server_name=self._credentials.plex_server_name,
                                      server_alt_name=self._credentials.plex_server_alt_name,
                                      server_number=1,
                                      credentials_folder="credentials/users",
                                      tautulli_info=self._credentials.tautulli,
                                      ombi_info=self._credentials.ombi
                                      )}
        return connectors
    ...
    def add_user_to_smallest_server(self, plex_username: str) -> bool:
        ...
        return smallest_server.add_user(plex_username=plex_username).__bool__()
