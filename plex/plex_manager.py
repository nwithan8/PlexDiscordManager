...
from databases.media_server_connector.table_connector import ConfigType
from modules import load_config
import modules.discord_helper as discord_helper
import modules.utils as utils
from databases.media_server_connector.database import EmbyUser, PlexUser, JellyfinUser, PlatformType
from modules.classes.basic_cog import *
from modules.load_config import get_database
from plex import plex_api as px_api
from plex.plex_api import PlexCredentials

async def is_not_admin(self, ctx: commands.Context):
    admin_id = self.get_a_setting("discordConfig", "AdminID")
    admin_role_name = self.get_a_setting("discordConfig", "AdminRoleName")

    if admin_id == ctx.author.id or admin_role_name in [role.name for role in ctx.author.roles]:
        return False

    await ctx.send("This command is locked to admins")
    return True
...
class PlexManager(BasicCog):
    ...
    @property
    def settings(self):
        return self._settings_manager.all_settings

    def get_a_setting(self, *args):
        item = self.settings
        count = 0
        for key in args:
            if not item:
                return None
            if count == len(args) - 1:  # we're looking for a property of an SQL entry
                item = getattr(item, key)
            else:  # we're grabbing a element of a dict
                item = item.get(key)
            count += 1
    ...
    def _plex_settings(self):
        return self._settings_manager.get_settings(config_type=ConfigType.Plex)

    @property
    def _ombi_settings(self):
        return self._settings_manager.get_settings(config_type=ConfigType.Ombi)

    @property
    def _tautulli_settings(self):
        return self._settings_manager.get_settings(config_type=ConfigType.Tautulli)

    def get_plex_api(self):
        database = get_database(media_server_type=PlatformType.Plex)
        plex_credentials = PlexCredentials(plex_settings_entry=self._plex_settings,
                                           ombi_settings_entry=self._ombi_settings,
                                           tautulli_settings_entry=self._tautulli_settings)
        return px_api.PlexConnections(plex_credentials=plex_credentials, database=database)
    ...
    def remove_user_from_database(self,
        ...
        if not user:
            user = plex_api.database.make_media_server_user(discord_id=discord_id, plex_username=plex_username)
    ...
    def check_blacklist(self,
        ...
        if self.get_a_setting("mediaServerConfig", "EnableBlacklist") and plex_api.database.on_blacklist(names_and_ids=to_check):
    ...
    def get_server_numbers_user_is_on(self,
        ...
        database_user_entries = self.get_user_entries_from_database(ctx=ctx, discord_id=discord_id,
                                                               plex_username=plex_username, first_only=False)
        if not database_user_entries:
    ...
    async def add_to_database_add_to_plex_add_role_send_dm(self,
                                                           ...
                                                           pay_method: str = None):
        if self.check_blacklist(ctx=ctx, discord_id=discord_id, plex_username=plex_username):
        ...
        if user_type == 'Trial':
            expiration_stamp = int(time.time()) + self.get_a_setting("trialConfig", "Length")
        new_database_entry = plex_api.database.make_media_server_user(plex_username=plex_username,
    ...
    async def remove_from_plex_remove_from_database_remove_role_send_dm(self,
        ...
        if not await discord_helper.remove_user_role(user=discord_user, role_name=role_name, reason=role_reason):
            return utils.StatusResponse(success=False,
                                        issue=f"Could not find {role_name} role to remove from Discord user.")
    ...
    async def check_trials(self, ctx: commands.Context):
        ...
        for trial_database_user in plex_api.database.expired_trials:
            await self.remove_from_plex_remove_from_database_remove_role_send_dm(ctx=ctx,
                                                                            role_name=self.get_a_setting("trialConfig", "RoleName"),
                    ...
                    if tautulli_user.username in winners_usernames:
                        if server.tautulli.get_last_time_user_seen(user_id=tautulli_user.username) < (
                            ...
                            if self.remove_from_plex_remove_from_database_remove_role_send_dm(ctx=ctx,
                                                                                         ...
                                                                                         role_name=self.get_a_setting("winnerConfig", "RoleName"),
                            ...
                            if user_stats.total_time < self.get_a_setting("winnerConfig", "MinutesRequiredPerWeek"):
                                if self.remove_from_plex_remove_from_database_remove_role_send_dm(ctx=ctx,
                                                                                             plex_username=tautulli_user.username,
                                                                                             role_name=self.get_a_setting("winnerConfig", "RoleName"),
                                                                                             role_reason="Inactive winner",
        ...
        for discord_member in discord_helper.get_users_without_roles(bot=self.bot,
            ...
            if discord_member.id not in plex_settings.EXEMPT_SUBS:
                await self.remove_from_plex_remove_from_database_remove_role_send_dm(ctx=ctx,
                                                                                ...
                                                                                role_name=self.get_a_setting("subscriberConfig", "InvitedRoleName"),
    ...
    # @commands.has_role(self.get_a_setting("discordConfig", "AdminRoleName"))
    async def pm_blacklist(self, ctx: commands.Context, add_or_remove: str,
        ...
        """
        if await is_not_admin(self, ctx):
            return
    ...
    async def pm_add(self, ctx: commands.Context, user: discord.Member, plex_username: str, server_number: int = None):
        ...
        Include optional server number to add to a specific server (if using multiple Plex servers)
        """
        if await is_not_admin(self, ctx):
            return
