...
def get_plex_credentials(ctx: commands.Context):
    ...
    plex_credentials_path = f"credentials/admin/{server_id}.json"
    try:
        with open(plex_credentials_path) as f:
            return json.load(f)
    except:
        # generate file and quit
        open(plex_credentials_path, 'a').close()
        print(f"Please fill out your credentials in '{plex_credentials_path}'.\nSee the sample file for a template.")
        exit(1)
...
class PlexManager(commands.Cog):
    ...
    @commands.has_role(plex_settings.DISCORD_ADMIN_ROLE_NAME)
    async def pm_import(self, ctx: commands.Context, user: discord.Member, plex_username: str, server_number: int = None):
        """
